		...
		<div class="auth-card">
			...
			{:else if step === 'otp'}
				<form class="auth-form" onsubmit={(e) => { e.preventDefault(); verifyOtpAndContinue(code); }}>
					...
					<button class="auth-submit auth-submit--quiet" type="button" disabled={resendCooldown > 0 || submitting} onclick={sendOtp}>
						{resendCooldown > 0 ? `Kirim ulang dalam ${resendCooldown}s` : 'Kirim ulang kode'}
					</button>
					<button class="auth-submit auth-submit--quiet" type="button" onclick={() => { step = 'email'; error = ''; notice = ''; }}>
						Ganti email
					</button>
				</form>
			{:else}
