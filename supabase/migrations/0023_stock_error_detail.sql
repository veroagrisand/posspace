-- ============================================================
-- posspace — migration 0023: error stok tidak cukup lebih jelas
-- process_transaction kini melempar INSUFFICIENT_STOCK beserta
-- NAMA bahan, jumlah yang dibutuhkan, dan stok yang ada — sehingga
-- kasir/pemilik langsung tahu bahan mana yang harus ditambah,
-- bukan hanya pesan generik "bahan tidak mencukupi".
-- ============================================================
  ...
  if p_items is not null then
    for v_item in select * from jsonb_array_elements(p_items) loop
      ...
      loop
        ...
        select i.stock_quantity, i.cost_per_unit, i.name, i.unit into v_stock, v_cost, v_ing_name, v_ing_unit
        ...
        if not v_pending then
          if v_stock < v_need then
            raise exception 'INSUFFICIENT_STOCK: % (butuh % % untuk % porsi, stok %)',
              coalesce(v_ing_name, 'bahan'), v_need, v_ing_unit, v_qty, v_stock;
          end if;
